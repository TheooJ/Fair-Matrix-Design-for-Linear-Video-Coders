...
for im_idx = 1:nb_im
    ...
    [G_mM, H_mM, N, MSE] = TER_Foreman_opti_mM(Lambda,P);                                          
    [G_lagr, H_lagr_1, H_lagr_2, N, MSE_1, MSE_2] = TER_Foreman_opti_lagr_new(Lambda,P); %G_lagr et G_mM cell array size 2
    ...
    for l=(1:3) %loop to compare both optimizations
        ...
        elseif l == 2 %Cas lagrangien optimisé pour channel 1
            G = G_lagr{1};
            H = H_lagr_1; % Les matrices de décodage doivent être transposées pour le Lag
        elseif l == 3 %Cas lagrangien optimisé pour channel 2
            G = G_lagr{2};
            H = H_lagr_2; %Transposée
        end
